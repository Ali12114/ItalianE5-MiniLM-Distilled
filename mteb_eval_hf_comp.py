...
class TruncatedE5:
    ...
    def encode(self, sentences,  **kwargs):
        ...
        sentences = [" ".join(sentence.split(" ")[:170])
...
class E5LargePCA:
    ...
    def encode(self, sentences, batch_size=32, **kwargs):
        if self.fair:
            sentences = [" ".join(sentence.split(" ")[:170])
                         for sentence in sentences]
        np_array = self.base_model.encode(
            sentences, batch_size=batch_size, convert_to_numpy=True, **kwargs)
        # idk why its not a numpy array in the first place
...
def track_evals(sentences: list[str], batch_size, teacher, student, eval_fns: list, device):
    ...
    for start_idx in range(0, len(sentences), batch_size):
        ...
        with torch.no_grad():
            ...
            t_emb = teacher.encode(
                batch, convert_to_tensor=True, device=device, normalize_embeddings=True)
            s_emb = student.encode(
                batch, convert_to_tensor=True, device=device, normalize_embeddings=True)
...
def get_italian_miniLM(model_path):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    student = SentenceTransformer(model_path).to(device)
...
if __name__ == "__main__":
    ...
    tasks = [
        mteb.tasks.Clustering.multilingual.SIB200ClusteringS2S.SIB200ClusteringFast(
        ).filter_languages(languages=["ita"], exclusive_language_filter=True),
        mteb.tasks.Reranking.multilingual.WikipediaRerankingMultilingual.WikipediaRerankingMultilingual(
        ).filter_languages(languages=["ita"], exclusive_language_filter=True),
        mteb.tasks.Retrieval.multilingual.BelebeleRetrieval.BelebeleRetrieval(
        ).filter_languages(languages=["ita"], exclusive_language_filter=True),
        mteb.tasks.Retrieval.multilingual.MultiLongDocRetrieval.MultiLongDocRetrieval(
        ).filter_languages(languages=["ita"], exclusive_language_filter=True),
        mteb.tasks.STS.multilingual.STS22CrosslingualSTS.STS22CrosslingualSTS(
        ).filter_languages(languages=["ita"], exclusive_language_filter=True),
        mteb.tasks.STS.multilingual.STSBenchmarkMultilingualSTS.STSBenchmarkMultilingualSTS(
        ).filter_languages(languages=["ita"], exclusive_language_filter=True),
        mteb.tasks.Retrieval.multilingual.WebFAQRetrieval.WebFAQRetrieval(
        ).filter_languages(languages=["ita"], exclusive_language_filter=True),
    ]
    eval = mteb.MTEB(tasks=tasks)
    # Please change this path to your own path where you have saved your checkpoint
    # Please replace it with your model path
